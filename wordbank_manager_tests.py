...
class WordbankManagerTest(unittest.TestCase):
    ...
    def test_get_words_with_letters(self):
        self.assertEqual(self.wordbank_manager.get_words_with_letters(['l','o']), ['flota', 'omlet'])
        self.assertEqual(self.wordbank_manager.get_words_with_letters('a'), ['pazur','flota', 'wydra', 'topaz', 'saper'])
        self.assertFalse('xxxxx' in self.wordbank_manager.get_words_with_letters('b'))
